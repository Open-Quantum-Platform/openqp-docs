...
| `theory.ccsd(reference="rhf", runtype=None, multiplicity=None, basis=None, nfzc=None, conv=None, maxit=None, ndiis=None, triples=False, **scf_keywords)` | `OpenQP` | Selects energy-only CCSD with an HF reference. `nfzc`, `conv`, `maxit`, and `ndiis` route to `[cc]`; every other keyword routes to `[scf]`. Passing a non-empty `functional`, or any `runtype` other than `energy`, raises. Development preview, not part of OpenQP 1.2.0. |
| `theory.ccsd_t(**kwargs)` | `OpenQP` | `theory.ccsd(...)` with `triples=True`, adding the perturbative triples correction. Takes the same arguments. Development preview, not part of OpenQP 1.2.0. |
...
| `theory(method, functional=None, basis=None, nstate=3, reference=None, **keywords)` | `OpenQP` | Backward-compatible string dispatcher for existing scripts. Use `"hf"`, `"dft"`, `"mp2"`, `"ccsd"`, `"ccsd(t)"`, `"tdhf"`, `"tddft"`, `"sf-tddft"`, or `"mrsf-tddft"`. `"cc"` is an alias for `"ccsd"`; `"ccsd-t"` and `"ccsdt"` for `"ccsd(t)"`. |
