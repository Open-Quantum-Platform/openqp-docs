...
Applications, rather than the copy inside the disk image. Studio 0.2.7 requires
...
Studio 0.2.7 has an ad-hoc signature checked during packaging, but it does not
...
The 0.2.7 desktop packages target Ubuntu 24.04 or a compatible newer Linux
...
```bash
VERSION=0.2.7  # replace when installing a later release
